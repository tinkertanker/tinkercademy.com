...
async function imagePropSVG(p) {
	...
	const x0 = p.x * W - pw / 2;
	const y0 = p.ground ? 0.76 * H - ph : p.y * H - ph / 2;
...
function darkGround(scene) {
	...
	return {
		...
		layers: `
			...
			<ellipse cx="${spotX * W}" cy="${0.79 * H}" rx="${0.3 * W}" ry="${0.045 * H}" fill="${PALETTE.paper}" opacity="0.05"/>
	...
	for (const p of out.props ?? []) {
		...
		if (i < layout.slots.length) {
			...
			if (BOTTOM_OFFSET[p.type]) {
				p.y = 0.76 - BOTTOM_OFFSET[p.type](p.w * W) / H;
			}
		}
	}
	return out;
